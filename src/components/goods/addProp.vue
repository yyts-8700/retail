  ...
  <div class='addProp'>
    ...
    <div class='body'>
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="200px" class="form">
        ...
        <el-form-item class='changeProp' label="属性是否可选: " prop="propertySelect">
          <el-radio v-model="ruleForm.propertySelect" :label="0" >唯一属性</el-radio>
          <el-radio v-model="ruleForm.propertySelect" :label="1" >单选属性</el-radio>
          <el-radio v-model="ruleForm.propertySelect" :label="2" >复选属性</el-radio>
          <p class="tips">选择"单选/复选属性"时，可以对商品该属性设置多个值，同时还能对不同属性值指定不同的价格加价，用户购买商品时需要选定具体的属性值。选择"唯一属性"时，商品的该属性值只能设置一个值，用户只能查看该值。</p>
        </el-form-item>
        <el-form-item class='props' label="属性值: " prop="propertyList">
        ...
        <el-form-item class='propSort' label='属性排序:' prop='propertyOrder'>
  ...
  mounted(){
    this.$axios.post("merchantGoodsStyle/merchant_goods_type_list").then((res)=>{
      ...
      if(this.$route.query.id){
        this.isAdd = false
        this.$axios.post("merchantGoodsProperty/merchant_goods_type_by_id",{
          id:this.$route.query.id
        }).then((res)=>{
          this.$set(this.ruleForm,'propertyName',res.data.data.propertyName)
          this.$set(this.ruleForm, 'styleId', res.data.data.styleId)
          this.$set(this.ruleForm, 'propertySelect', res.data.data.propertySelect)
          if(res.data.data.propertyList.length>0){
            this.dynamicTags = res.data.data.propertyList.split(',')
          }
          this.$set(this.ruleForm, 'propertyOrder', res.data.data.propertyOrder)
        })
      }
    })
  },
    ...
    submit(val){
      this.$refs[val].validate((valid)=>{
        if(valid){
          ...
          if(this.isAdd){
            this.$axios.post('merchantGoodsProperty/merchant_goods_type_add',this.ruleForm).then(()=>{
              this.$message({
                  type:'success',
                  message:'操作成功'
                });
            ...
            this.$axios.post('merchantGoodsProperty/merchant_goods_type_update',this.ruleForm).then(()=>{
              this.$message({
                  type:'success',
                  message:'操作成功'
                });
