  ...
  <div class='refund'>
    ...
    <div class='body'>
        <div class='menu-btn-wrapper'>
            ...
            <el-button class='menu-btn' :class='isActive==4?"active":""' @click='changeList(4)'>
                已处理(<em class='number'>{{all_return}}</em>)
            </el-button>
        ...
        <el-table @selection-change='selectionChange' ref='table' class='order-list' :data='tableData' :header-cell-style="headerStyle" :cell-style="tdStyle">
	        ...
	        <el-table-column label="服务单号" width='200'>
	        ...
	        <el-table-column label="操作">
	          <template slot-scope="scope">
	            <span v-if="scope.row.status == 2" class="table-btn"
	                  @click="$router.push('/orders/refund/refundDetail?id=' + scope.row.id)">查看详情</span>
	            <span v-else-if="scope.row.status == 5" class="table-btn" @click="remove([scope.row.id])">删除</span>
	          </template>
	        </el-table-column>
        </el-table>
